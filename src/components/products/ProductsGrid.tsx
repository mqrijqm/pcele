    ...
    <>
      <div className="reveal mb-8">
        <div className="relative max-w-lg">
          <Search
            aria-hidden="true"
            className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#885B27]"
          />
          <input
            type="search"
            value={query}
            ...
            className="min-h-12 w-full border-x-0 border-b border-t-0 border-[#885B27]/20 bg-transparent py-2 pl-11 pr-10 text-sm text-[#885B27] outline-none transition-colors placeholder:text-[#885B27] focus:border-[#EEC660] rounded-[0.6rem]"
          />
        </div>
      </div>

      <div className="reveal mb-12 flex flex-col gap-5 border-b border-[#885B27]/15 pb-7 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-3">
          <div className="mr-1 flex items-center gap-1.5 text-sm text-[#885B27]">
          ...
          {filters.map((filter) => (
            <button
              ...
              className={`min-h-10 px-3 text-sm font-semibold transition-colors duration-300 ${
                category === filter.id
                  ? 'border-b border-[#885B27] text-[#885B27]'
                  : 'border-b border-transparent text-[#885B27] hover:border-[#885B27]/30 hover:text-[#885B27]'
              }`}
            >
              {filter.label}
            </button>
        ...
        <div className="flex items-center gap-2">
          <ArrowUpDown className="h-4 w-4 text-[#885B27]" />
          <div className="relative">
            <select
              value={sort}
              onChange={(event) => setSort(event.target.value as SortOption)}
              aria-label={t('products.sort.label')}
              className="min-h-10 appearance-none border border-[#885B27]/20 bg-transparent py-2 pl-4 pr-9 text-sm text-[#885B27] outline-none transition-colors focus:border-[#EEC660] rounded-[0.6rem]"
            >
            ...
            <ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#885B27]" />
          </div>
        </div>
      </div>
      ...
      {visible.length === 0 ? (
        <div className="py-24 lg:py-32 text-center">
          <p className="text-2xl text-[#885B27]">{t('products.grid.noResults')}</p>
          <p className="mt-3 text-sm text-[#885B27]">
          ...
          <button
            ...
            className="mt-6 border-b border-[#885B27] pb-1 text-sm font-semibold text-[#885B27]"
          >
