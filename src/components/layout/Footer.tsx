...
export default function Footer({ locale }: { locale: Locale }) {
  ...
  return (
    <footer className="bg-[#73552E] text-[#F5E8D8]">
      <div className="container pt-28 lg:pt-40">
        ...
        <div className="reveal grid gap-14 sm:grid-cols-2 lg:grid-cols-[1fr_auto]">
          <div className="max-w-sm">
            ...
            <p className="mt-6 text-base leading-7 text-[#F5E8D8]/90">{t('footer.tagline')}</p>
          </div>
          ...
          <div className="flex flex-col gap-8 sm:flex-row sm:gap-14 lg:gap-20">
              ...
              {navLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    ...
                    className="text-sm text-[#F5E8D8]/90 transition-colors hover:text-[#F5E8D8]"
                  >
                    {t(link.key)}
                  </Link>
                </li>
            ...
            <div className="space-y-2.5 text-sm">
              <p className="text-[#F5E8D8]/90">{t('contact.info.address')}</p>
              <p>
                <a
                  href="tel:+38766030550"
                  className="text-[#F5E8D8] transition-colors hover:underline hover:decoration-[#C79A3B] hover:decoration-2 hover:underline-offset-4"
                >
              ...
              <div className="flex gap-5 pt-1.5">
                {socials.map(({ href, label }) => (
                  <a
                    ...
                    className="text-sm text-[#F5E8D8]/90 underline-offset-4 transition-colors hover:underline hover:decoration-[#C79A3B] hover:decoration-2 hover:underline-offset-4 hover:underline"
                  >
                    {label}
                  </a>
                ))}
        ...
        <div className="mt-14 flex flex-col items-start justify-between gap-3 border-t border-[#F5E8D8]/15 pt-5 text-xs text-[#F5E8D8]/90 md:flex-row md:items-center">
          <p>{t('footer.copyright', { year })}</p>
          <div className="flex items-center gap-5">
            <Link
              href={localeHref(locale, '/privacy')}
              className="transition-colors hover:text-[#F5E8D8]"
            >
              {t('footer.privacy')}
            </Link>
            <Link
      ...
      <div className="mt-16 w-full px-4 pb-6 sm:px-6 lg:px-8">
        <p className="text-center text-[clamp(0.58rem,1.15vw,1rem)] font-semibold uppercase tracking-[0.4em] text-[#F5E8D8]">
          Pčelarstvo
        </p>
        <p className="-mt-[0.22em] select-none text-center font-display text-[clamp(4rem,33.5vw,30rem)] font-normal leading-[1.22] tracking-[-0.02em] text-[#F5E8D8]">
          Jevtić
        </p>
      </div>
    </footer>
